    ...
    try:
        if request.method == 'POST':
            ...
            if file and allowed_file(file.filename):
                ...
                cloudSaveFilename = str(session["uid"]) + "_" + str(time.time()).replace('.','') + "_" + userFileName  #example: 12_1_example.jpg
                ...
                return render_template("upload_management.html", uploadImagePath= create_presigned_url_expanded(cloudSaveFilename), fileName=fileName, person_name=personName,info_msg=info_msg)
    ...
    if ('authenticated' in session) and ('username' in session):
        ...
        if session['authenticated'] == True:
            ...
            # if there exists uploaded image:
                ...
                return render_template("file_management.html",fileNumber=fileNumber, dictList=dictList, info_msg = info_msg,error_msg = error_msg)
