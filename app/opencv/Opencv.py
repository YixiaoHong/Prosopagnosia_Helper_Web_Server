...
def imageProcess(imgName, img):
    ...
    #cv2.imshow ("Display window", originalImgCopy)
    (H, W) = image.shape[:2]
    (newW, newH) = (320, 320)
    rW = W / float(newW)
    rH = H / float(newH)
    ...
    # net = cv2.dnn.readNet("/ece1779-image-processing/app/opencv/frozen_east_text_detection.pb")
    ...
    # out = Image.fromarray(originalImgCopy, "RGB")
    # b = io.BytesIO()
    # out.save(b, "png")
    # b.seek(0)
    # cv2.imwrite(folderPath + saveImgName, originalImgCopy)
    ...
    b = cv2.imencode('.jpg', originalImgCopy)[1].tostring()#todo ext???
    store_file(imgName, b)
